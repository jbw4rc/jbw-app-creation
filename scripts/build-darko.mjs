...
for (const p of parts) {
  ...
  if (!prev || games >= prev.games) byId.set(id, { id: +id, name, dpm, odpm, ddpm, salary, value, surplus, rank, games, age, pos, posNum, min, decline });
}
...
writeFileSync(
  ...
  `// AUTO-GENERATED DARKO Daily Plus-Minus (DPM) from darko.app.\n` +
    ...
    `export interface DarkoInfo { name: string; dpm: number; odpm: number | null; ddpm: number | null; salary: number | null; value: number | null; surplus: number | null; rank: number | null; age: number | null; pos: string | null; posNum: number | null; min: number | null; decline: (number | null)[]; }\n\n` +
